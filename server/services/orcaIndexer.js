/**
 * Orca Indexer - Pool data and position tracking for Orca Whirlpools
 * Replaces LP Agent API with Orca-specific data layer
 */
...
function generatePoolStats(pool) {
  ...
  return { ...pool, tvl: baseTvl, vol_24h, price_24h_change, organic_score, liquidity: baseTvl * 1.2, current_price, agentScore: score };
}

class OrcaIndexer {
  constructor() {
    this.pools = ORCA_POOLS.map(generatePoolStats);
  ...
  getTopPools(limit = 20) {
    return this.pools
      .sort((a, b) => b.agentScore - a.agentScore)
      .slice(0, limit)
      .map(p => ({ ...p, daily_yield_pct: ((p.vol_24h * p.fee_rate) / p.tvl) * 100 }));
  }

  getPool(poolAddress) {
    return this.pools.find(p => p.address === poolAddress);
  }
  ...
  getPositionsForOwner(owner) {
    ...
    return [
      { id: 'pos_' + prefix + '_1', pool_address: 'HJPjoWUrhoZzkNfRpHuie63vDCx6cSiq4rEJ9wA9Di9', token0_symbol: 'SOL', token1_symbol: 'USDC', current_value_usd: 5200, pnl: { percent: 4.2, usd: 210 }, dpr: 0.0018, inRange: true, uncollected_fees_usd: 12.40, age_days: 5, isHealthy: true, tick_lower: -10000, tick_upper: 10000, liquidity: 450000000, fee_tier: 0.0005 },
      { id: 'pos_' + prefix + '_2', pool_address: '4fuUiYxTQdFKL1q5aY9g1KhYrC5eM5qLz3qFq1K6Z3nS', token0_symbol: 'USDC', token1_symbol: 'USDT', current_value_usd: 15000, pnl: { percent: 0.8, usd: 120 }, dpr: 0.0006, inRange: true, uncollected_fees_usd: 8.20, age_days: 12, isHealthy: true, tick_lower: -500, tick_upper: 500, liquidity: 15000000000, fee_tier: 0.0001 },
    ].map(p => ({ ...p, isHealthy: p.inRange && p.dpr > 0 && p.pnl.percent > -5 }));
  }

  getPortfolioOverview(owner) {
    const positions = this.getPositionsForOwner(owner);
    ...
    return { total_value_usd, total_pnl_usd, total_pnl_percent, avg_dpr, positions_count: positions.length };
  }

  getFeeTierRecommendation(poolName, volatility7d = 5) {
  ...
  getILProjection(positionId, priceChangePct = 10) {
    const ilPct = -(priceChangePct ** 2) / 400;
    const position = this.getPositionsForOwner('demo').find(p => p.id === positionId);
    const currentValue = position ? position.current_value_usd : 5000;
    const projectedValue = currentValue * (1 + ilPct / 100);
    let hedgeSuggestion = null;
    if (Math.abs(priceChangePct) > 15) {
      hedgeSuggestion = 'Consider a perp hedge or wider tick range to reduce IL exposure';
    }
    return { position_id: positionId, price_change_pct: priceChangePct, projected_il_pct: ilPct, projected_value_usd: projectedValue, hedge_suggestion: hedgeSuggestion };
  }

  getPoolDepth(poolAddress) {
    const pool = this.getPool(poolAddress);
    if (!pool) return null;
    const ticks = [];
    const currentTick = 0;
    for (let i = -20; i <= 20; i++) {
      const dist = Math.exp(-(i ** 2) / 20);
      ticks.push({ tick: currentTick + i * pool.tick_spacing, liquidity: pool.liquidity * dist * (0.5 + Math.random() * 0.5), price: pool.current_price * (1 + i * 0.001) });
    }
    return { poolAddress, ticks, current_price: pool.current_price };
  }
}

module.exports = { OrcaIndexer };